...
import { getBlogsByCategory, getSettings } from "@/lib/data"
import { ArrowLeft } from "lucide-react"

export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  return {
    title: `${params.slug.replace(/-/g, " ")} Blog Posts`,
...
export default async function BlogCategoryPage({ params }: { params: { slug: string } }) {
  ...
  const categoryName = params.slug.replace(/-/g, " ").charAt(0).toUpperCase() + params.slug.replace(/-/g, " ").slice(1)
        ...
        <div className="container py-12 md:py-16">
          ...
          <div className="mb-12">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              {categoryName} <Badge className="ml-2">Category</Badge>
            </h1>
